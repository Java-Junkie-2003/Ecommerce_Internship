...
const authenticatorForRoleAdmin = asyncHandler(async (req, res, next) => {
    const userId = req.headers[HEADER.CLIENT_ID]
    if (!userId) throw new AuthFailureError("Invalid request!")
    const keyStore = await keyTokenService.findByUserId(userId)
    if (!keyStore) throw new NotFoundError("Keystore not found")
    ...
    try {
        const decodedUser = jwt.verify(accessToken, keyStore.publicKey)
        if (userId !== decodedUser.userId) throw new AuthFailureError("Invalid user");
        if (!decodedUser.roles.includes('ADMIN')) throw new ForbiddenError("Fobbiden request !!")
        return next()
    } catch (error) {
        throw new AuthFailureError('Invalid token')
    }
